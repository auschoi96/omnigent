...
@pytest.mark.asyncio
@pytest.mark.parametrize("earlier_action", [PolicyAction.ALLOW, PolicyAction.ASK])
@pytest.mark.parametrize("read_only", [False, True])
async def test_denying_policy_spec_identifies_actual_policy_with_duplicate_names(
    engine: PolicyEngine,
    earlier_action: PolicyAction,
    read_only: bool,
) -> None:
    """A later denying policy can share an earlier policy's display name."""
    from tests.runtime.policies.conftest import make_fixed_policy

    earlier = make_fixed_policy(name="shared_name", action=earlier_action)
    denying = make_fixed_policy(name="shared_name", action=PolicyAction.DENY)
    engine.policies = [earlier, denying]
    assert engine.denying_policy_spec is None

    result = await engine.evaluate(
        EvaluationContext(phase=Phase.REQUEST, content="hello", tool_name=None),
        read_only=read_only,
    )

    assert result.action == PolicyAction.DENY
    assert result.deciding_policy == "shared_name"
    assert engine.spec_for(result.deciding_policy) is earlier.spec
    assert engine.denying_policy_spec is denying.spec


@pytest.mark.asyncio
@pytest.mark.parametrize("next_action", [PolicyAction.ALLOW, PolicyAction.ASK])
async def test_denying_policy_spec_cleared_after_non_deny_evaluation(
    engine: PolicyEngine,
    next_action: PolicyAction,
) -> None:
    """A later allowed or approval-gated phase must not reuse a previous DENY."""
    from omnigent.spec.types import PhaseSelector
    from tests.runtime.policies.conftest import make_fixed_policy

    denying = make_fixed_policy(
        name="request_guard",
    ...
    result = await engine.evaluate(
        EvaluationContext(phase=Phase.RESPONSE, content="reply", tool_name=None)
    )

    assert result.action == next_action
    assert engine.denying_policy_spec is None
