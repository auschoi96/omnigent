...
@pytest.mark.asyncio
async def test_terminate_sandbox_for_run_only_tears_down_sandboxes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The completion teardown terminates a sandbox host, and leaves a plain host alone."""
    calls: list[Any] = []
    ...
    class _HostStore:
        ...
        def get_host(self, _hid: str) -> Any:
            return self._host

    cfg = _FakeSandboxConfig()
    # A managed sandbox host (sandbox_id set) is torn down.
    sandbox_host = _FakeHost("host_1", "u", sandbox_provider="modal", sandbox_id="sbx_1")
    await fire_mod._terminate_sandbox_for_run(
        "conv_1", 0, _ConvStore(_Conv("host_1")), _HostStore(sandbox_host), cfg
    )
    assert calls == [sandbox_host]

    # A plain connected host (no sandbox_id) is left alone.
    calls.clear()
    await fire_mod._terminate_sandbox_for_run(
        "conv_2", 0, _ConvStore(_Conv("host_2")), _HostStore(_FakeHost("host_2", "u")), cfg
    )
    assert calls == []
